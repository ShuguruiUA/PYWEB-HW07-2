...
from conf.models2 import Teacher, Student, Group, Subject
...
def create_record(model, name, model_name=None):
    ...
    if model == "Grade":
        ...
        model_name = eval(model)(fullname=name)
    elif model == "Student":
        group_ids = session.query(Group.id).all()
        model_name = eval(model)(fullname=name, group_id=random.choice(group_ids)[0])
    elif model == "Group":
